...
export default function MenteeProfileScreen({ navigation }) {
  const [name, setName] = useState('');
  const [introduction, setIntroduction] = useState('');

  const isFormValid = name.trim().length > 0 && introduction.trim().length > 0;
      ...
      <View style={styles.progressContainer}>
        <View style={styles.progressIconsContainer}>
          ...
          <View style={styles.progressIcon}>
            <FolderIcon2 width={20} height={20} color="#9B9BAA" />
          </View>
          <View style={styles.progressLine} />
        ...
        <View style={styles.progressLabelsContainer}>
          ...
          <Text style={[styles.progressLabel, { left: 95.134 - 35 }]} numberOfLines={1}>학력/목표</Text>
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            if (isFormValid && navigation) {
              ...
              navigation.navigate('MenteeProfileStep2', { name });
            }
          }}
